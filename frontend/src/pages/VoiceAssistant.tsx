...
const WELCOME: Record<string, string> = {
  Hindi:   'नमस्ते! मैं AgriVerse AI हूँ 🌾 खेती से जुड़ा कोई भी सवाल पूछें — रोग, मौसम, मंडी भाव, सरकारी योजना। माइक दबाकर बात भी कर सकते हैं!',
  Marathi: 'नमस्कार! मी AgriVerse AI आहे 🌾 शेतीशी संबंधित कोणताही प्रश्न विचारा. मायक्रोफोन दाबून बोलूही शकता!',
...
export default function VoiceAssistant() {
  ...
  const speak = useCallback((text: string) => {
    if (!autoSpeak || !synth.current) return;
    stopSpeaking();
    const clean = text.replace(/[\u{1F000}-\u{1FFFF}]/gu, '').trim();
    const utt = new SpeechSynthesisUtterance(clean);
    utt.lang = LANG_CODE[activeLang];
    utt.rate = 0.9;
    utt.pitch = 1;
    const voices = synth.current.getVoices();
    const v = voices.find(v => v.lang.startsWith(LANG_CODE[activeLang].split('-')[0]));
    if (v) utt.voice = v;
  ...
  const handleMicClick = () => {
    ...
    if (!SR) { setError('Chrome browser use karo voice ke liye।'); return; }
    stopSpeaking();
    const rec = new SR();
    ...
    rec.onresult = (e: any) => {
      ...
      setTimeout(() => { setMicState('idle'); sendMessage(t); }, 300);
    };
    rec.onerror = (e: any) => {
      setMicState('idle');
      if (e.error === 'not-allowed') setError('Microphone permission deny hai। Browser settings check karo।');
  ...
  const getMicStyle = () => ({
    idle:       'bg-gradient-to-br from-green-900/60 to-green-800/40 hover:from-green-700/60 border-2 border-green-500/30 hover:border-green-400/50',
  ...
  const getMicLabel = () => {
    if (micState === 'listening')  return activeLang === 'Hindi' ? '🔴 सुन रहा हूँ... (रोकें)' : activeLang === 'Marathi' ? '🔴 ऐकतोय...' : '🔴 Listening...';
    ...
    <div className="min-h-screen bg-[#050c08] text-green-50 flex flex-col font-sans">
      ...
      <header className="bg-[#0f2318] border-b border-green-900/50 p-4 shrink-0">
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => { setAutoSpeak(p => !p); stopSpeaking(); }}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs border transition-all ${autoSpeak ? 'bg-green-500/20 border-green-500/40 text-green-300' : 'bg-gray-900/40 border-gray-700/40 text-gray-400'}`}>
            ...
            <div className="flex items-center bg-[#050c08] p-1 rounded-lg border border-green-900/50">
              ...
              {LANGUAGES.map(lang => (
                <button key={lang} onClick={() => setActiveLang(lang)}
                  className={`px-3 py-1.5 text-sm rounded-md transition-all ${activeLang === lang ? 'bg-green-600 text-white' : 'text-green-400/70 hover:text-green-200'}`}>
      ...
      <main className="flex-1 container mx-auto p-4 flex flex-col lg:flex-row gap-6 overflow-hidden">
              ...
              <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex gap-3 max-w-[85%] md:max-w-[75%] ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`group flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                    <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${msg.sender === 'user' ? 'bg-green-600 text-white rounded-tr-none' : 'bg-[#152e20] border border-green-900/50 text-green-50 rounded-tl-none'}`}>
                      {msg.text}
                    </div>
                    <div className={`flex items-center mt-1 gap-2 text-xs text-green-400/40 ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                      ...
                      {msg.sender === 'ai' && (
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                          <button onClick={() => speak(msg.text)} className="p-1 hover:bg-green-900/30 rounded"><Volume2 className="w-3 h-3" /></button>
                          <button onClick={() => navigator.clipboard.writeText(msg.text)} className="p-1 hover:bg-green-900/30 rounded"><Copy className="w-3 h-3" /></button>
        ...
        <div className="w-full lg:w-[360px] flex flex-col gap-5 shrink-0">
          ...
          <div className="bg-[#0f2318] border border-green-900/30 rounded-2xl p-6 flex flex-col items-center justify-center relative overflow-hidden min-h-[280px]">
            ...
            <p className="text-sm font-medium text-green-300 text-center">{getMicLabel()}</p>
            ...
            <div className="h-10 mt-5 flex items-end gap-1">
              {[...Array(9)].map((_, i) => (
                <div key={i} className={`w-2 rounded-t transition-all duration-100 ${micState === 'listening' ? 'bg-red-400' : micState === 'processing' ? 'bg-amber-400' : isSpeaking ? 'bg-green-400' : 'bg-green-900/30'}`}
                  style={{ height: (micState === 'listening' || isSpeaking) ? `${15 + Math.random() * 85}%` : '4px' }} />
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {SUGGESTIONS[activeLang].map(s => (
                <button key={s} onClick={() => sendMessage(s)} disabled={isLoading}
                  className="px-3 py-1.5 bg-green-900/20 hover:bg-green-800/40 border border-green-500/20 hover:border-green-500/50 rounded-full text-xs text-green-300 transition-all disabled:opacity-40">
                  {s}
                </button>
              ))}
            </div>
          ...
          <div className="bg-[#0a1810] border border-green-900/20 rounded-2xl p-4">
            ...
            <ul className="text-xs text-green-900/70 space-y-1.5">
              ...
              <li>⌨️ {activeLang === 'Hindi' ? 'या टाइप करके Enter दबाएं' : activeLang === 'Marathi' ? 'किंवा टाइप करून Enter' : 'Or type and press Enter'}</li>
              <li>🔊 {activeLang === 'Hindi' ? 'AI का जवाब बोलकर सुनाएगा' : activeLang === 'Marathi' ? 'AI उत्तर बोलून सांगेल' : 'AI will speak the answer'}</li>
              <li>🌐 {activeLang === 'Hindi' ? 'ऊपर से भाषा बदलें' : activeLang === 'Marathi' ? 'वर भाषा बदला' : 'Change language above'}</li>
            </ul>
          </div>
        </div>
      </main>
